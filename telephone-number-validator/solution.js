...
// a valid US number. The following are examples of valid formats for 
...
const telephoneCheck = (str) => {
  const phoneValidatorRegex = /^(1\s?)?\s?(\d{3}|\(\d{3}\))[\s\-]?\d{3}[\s\-]?\d{4}$/
  return phoneValidatorRegex.test(str)
}
